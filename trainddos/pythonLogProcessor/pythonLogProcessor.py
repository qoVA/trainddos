...
#this function calls two other functions and creates threads so they appear to run concurr$

    process_snort = pr(target = snort_log_fetch , name ='snort_process', args=(redis_fetched_queue_sys,))
    process_sys_monitor = pr(target = sys_monitor_fetch, name ='sys_monitor_process', args=(redis_fetched_queue_snort,))

     
	process_snort.start()
    process_sys_monitor.start()
    ...
    while True:
		...
		redis.web.redis.setex(seq, json.dumps(processed_logs),'30')
